...
A correlation map with a heatmap is generated.

```{r}
feature_data_matrix <- data #%>% select(-diagnosis)
...
Taking into consideration the previous heatmap, we analyze the features against the response variable diagnosis
and we can see the most correlated are those with a value higher than 0.6.
Mean values of texture, smoothness, symmetry or fractual dimension does not show a 
particular preference of one diagnosis over the other. 
...
even though those last two variable are correlated, we decide not to drop any more feature
