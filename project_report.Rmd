...
## A graphical way to see the featueres related to diagnosis

In the previous paragraph we saw which features related to the target variable diagnosis.
...
It is possible to use the correlation among features to reduce their number, encreasing the explanability of the multilinear regression model we will face soon.

## Covariance and Correlation

followe the scatter plot and the analysis above we can explore more the variables left.
...
ggarrange(plotlist = plot_first_list, ncol = 3, nrow = 5)
...
from the plot and the correlation values we can see a very strong correlation between all the features in this table so we can drop them all except for one. 
...
ggarrange(plotlist = plot_remained_list, ncol = 3, nrow = 5)
...
We use the four selected features for the multiple linear regression model
...
Let we check if the residuals of our linear regression are normally distributed. 
...
Let we see if the Shapiro–Wilk test confirms that.
...
Although the test returns a very high coefficient, having a p-value < 0.05 we can't accept the null Hypotesys and have to conclude
